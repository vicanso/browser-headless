...
// Re-export the alias for submodules that type-check against CDP security details.
#[allow(unused_imports)]
pub(crate) use chromiumoxide::cdp::browser_protocol::network::SecurityDetails as CdpSecurityDetails;

mod markdown;
mod security;

// Security pure functions used across collect / markdown.
use security::{
    build_cors_issues, build_security_audit, extract_security_headers, extract_tls_info,
    format_remote_ip, format_tls_expiry,
};
...
/// Record a capture-pipeline stage duration for Prometheus.
fn record_stage(stage: &'static str, started: Instant) {
    metrics::histogram!(
...
/// MIME maps to "other".
pub(crate) fn mime_bucket(mime: &str) -> &'static str {
    let m = mime.to_ascii_lowercase();
...
pub struct CollectCaptures {
    /// Lean content-only path: only track Document status/final URL +
    /// lifecycle gates; skip full resource inventory, TLS inventory,
    /// security_audit, and exception rollups. Used when `content_only`.
    pub lean: bool,
...
/// Document responses for `document_timing` (status / final URL), ignore the
/// full resource inventory and analytical rollups. Dramatically cuts memory
/// and per-event CPU on heavy pages.
async fn collect_summary_lean(
    page: &Page,
    url: &str,
    timeout: Duration,
            ...
            Some(ev) = lifecycle_stream.next() => {
                ...
                match ev.name.as_str() {
                    ...
                    "load" => {
                        ...
                        load_at.get_or_insert_with(Instant::now);
                    }
                    "networkIdle" => {
                        idle_at.get_or_insert_with(Instant::now);
                    }
                    _ => {}
                }
            }
            Some(ev) = response_stream.next() => {
                ...
                if let Some(pos) = pending_patterns.iter().position(|p| resp_url.contains(*p)) {
                    if !(200..400).contains(&status) {
                        return Err(Error::UpstreamFailure { status, url: resp_url });
                    }
                    ...
                    tracing::debug!(
                        ...
                        "wait_for_request matched (lean)",
                    );
                }
                // Document only — status + final URL for content_only envelope.
                if matches!(ev.r#type, ResourceType::Document) {
                    let s = status as u32;
                    if s < 400 {
                        let from_cache = ev.response.from_disk_cache.unwrap_or(false)
                            || ev.response.from_service_worker.unwrap_or(false)
                            || ev.response.from_prefetch_cache.unwrap_or(false);
                        let protocol = ev.response.protocol.clone().unwrap_or_default();
    ...
    let to_ms = |ts: Option<f64>| -> u32 {
        match (init_ts, ts) {
            (Some(i), Some(t)) if t >= i => ((t - i) * 1000.0) as u32,
            _ => 0,
        }
    };
...
pub async fn collect_summary(
    ...
    // Lean content-only path: skip full resource inventory / TLS inventory /
    // exception streams. Still tracks Document status + final URL for the
    // compact content response, and still honours wait_for_request + gates.
    if captures.lean {
        return collect_summary_lean(page, url, timeout, wait_for_request, wait_until_load).await;
    }
...
pub(crate) fn is_real_resource(url: &str) -> bool {
...
pub async fn capture(
    ...
    .await?;
    record_stage("collect", t_collect);
    tracing::debug!(
        stage = "collect",
    ...
    record_stage("capture", t_capture);
    tracing::debug!(
        stage = "capture",
...
#[cfg(test)]
mod tests {
    use super::security::{build_cors_issues, parse_csp, parse_hsts};
    use super::*;

    #[test]
    fn mime_bucket_classifies_common_types() {
        assert_eq!(mime_bucket("text/html; charset=utf-8"), "html");
        assert_eq!(mime_bucket("application/javascript"), "javascript");
        assert_eq!(mime_bucket("text/css"), "css");
        assert_eq!(mime_bucket("image/png"), "image");
        assert_eq!(mime_bucket("font/woff2"), "font");
        assert_eq!(mime_bucket("application/json"), "json");
        assert_eq!(mime_bucket("application/octet-stream"), "other");
    }

    #[test]
    fn is_real_resource_filters_pseudo_urls() {
        assert!(!is_real_resource("data:text/html,hi"));
        assert!(!is_real_resource("blob:https://x/1"));
        assert!(!is_real_resource("about:blank"));
        assert!(is_real_resource("https://example.com/a.js"));
    }
